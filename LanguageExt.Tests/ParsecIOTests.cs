...
namespace LanguageExt.Tests
{
    public class ParsecIOTests
    {
        ...
        [Fact]
        public void Indent1FailTest()
        {
            var toks    = Seq(new Tok(1, 1, 'a'), new Tok(2, 1, 'b'), new Tok(2, 2, 'c'), new Tok(3, 2, '1'), new Tok(1, 3, 'd'), new Tok(1, 4, '1'));
            var letter  = satisfy<Tok>(t => Char.IsLetter(t.Val));
            var digit   = satisfy<Tok>(t => Char.IsDigit(t.Val));
            var letters = many1(attempt(letter));
            var digits  = many1(attempt(digit));
            ...
            var res = (string)block.Parse(toks, t => new Pos(t.Ln - 1, t.Col - 1)).ToEither();

            Assert.True(res == "error at (line 2, column 3): unexpected extra tokens in indented section that can't be parsed");
        }
    }
}

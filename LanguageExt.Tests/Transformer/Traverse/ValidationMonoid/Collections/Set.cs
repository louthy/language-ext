...
namespace LanguageExt.Tests.Transformer.Traverse.ValidationMonoid.Collections
{
    public class Set
    {
        ...
        {
            var ma = Set(Fail<MSeq<Error>, Seq<Error>, int>(Seq(Error.New("failed"))), Success<MSeq<Error>, Seq<Error>, int>(12));
            var mb = ma.Traverse(identity);
            Assert.Equal(Fail<MSeq<Error>, Seq<Error>, Set<int>>(Seq(Error.New("failed"))), mb);
        }
    }
}

...
public partial class TrieSet
{
    ...
    public ref struct FoldState
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Peek<EqK, K>(ref FoldState state, out TrieSet<EqK, K>.Node item)
            where EqK : Eq<K>
        {
            var top  = state.Top - 1;
            var span = MemoryMarshal.CreateSpan(ref state.NodeStack0, StackDepth);
            item = span[top] as TrieSet<EqK, K>.Node ?? throw new InvalidOperationException("Invalid trie-map item type");
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static bool Step<EqK, K>(ref FoldState state, out K node)
            where EqK : Eq<K>
        {
            ref var top = ref state.Top;
            while (true)
            {
                ...
                Peek<EqK, K>(ref state, out var n);

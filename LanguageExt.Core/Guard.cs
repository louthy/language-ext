...
namespace LanguageExt
{
    ...
    public readonly struct Guard<E, A>
    {
        ...
        /// <summary>
        /// Natural transformation to `EitherUnsafe`
        /// </summary>
        public EitherUnsafe<E, Unit> ToEitherUnsafe() =>
            Flag
                ? Right(unit)
                : Left(OnFalse());
        ...
        /// Monadic binding support for `Either`
        /// </summary>
        public EitherUnsafe<E, B> Bind<B>(Func<Unit, EitherUnsafe<E, B>> f) =>
            Flag
                ? f(default)
                : LeftUnsafe(OnFalse());
        ...
        /// Monadic binding `SelectMany` extension for `Guard`
        /// </summary>
        public EitherUnsafe<E, C> SelectMany<B, C>(
            Func<Unit, EitherUnsafe<E, B>> bind, Func<Unit, B, C> project) =>
            Flag
                ? bind(default).Map(b => project(default, b))
                : LeftUnsafe(OnFalse());

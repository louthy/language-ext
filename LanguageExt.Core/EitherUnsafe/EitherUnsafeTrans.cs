...
namespace LanguageExt
{
    ...
    public static partial class Prelude
    {
        ...
        [Pure]
        public static IEnumerable<EitherUnsafe<LRet, RRet>> bibindT<L, R, LRet, RRet>(IEnumerable<EitherUnsafe<L, R>> self, Func<R, EitherUnsafe<LRet, RRet>> Right, Func<L, EitherUnsafe<LRet, RRet>> Left) =>
            self.Map(x => x.BiBind(Right, Left));

        [Pure]
        public static IEnumerable<EitherUnsafe<L, R>> bifilterT<L, R>(IEnumerable<EitherUnsafe<L, R>> self, Func<R, bool> Right, Func<L, bool> Left) =>
            self.Map(x => x.BiFilter(Right, Left));
    }
}

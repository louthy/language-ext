...
public static partial class Prelude
{
    ...
    /// <returns>RWS monad</returns>
    [Pure]
    public static RWS<MonoidW, R, W, S, A> RWS<MonoidW, R, W, S, A>(W output, S state, A value) where MonoidW : Monoid<W> => (_, _) =>
        (output, state, value);
    ...
    /// <returns>RWS monad</returns>
    [Pure]
    public static RWS<MonoidW, R, W, S, A> RWSFail<MonoidW, R, W, S, A>(Error error) where MonoidW : Monoid<W> => (_, _) =>
        error;
    ...
    public static RWS<MonoidW, R, W, S, Unit> put<MonoidW, R, W, S, A>(S state)
        where MonoidW : Monoid<W> =>
        MRWS<MonoidW, R, W, S, Unit>.Put(state);
    ...
    public static RWS<MonoidW, R, W, S, A> gets<MonoidW, R, W, S, A>(Func<S, A> f)
        where MonoidW : Monoid<W> => 
        (env, state) =>
            MRWS<MonoidW, R, W, S, A>.Return(input => f(input.State))(env, state);
    ...
    public static RWS<MonoidW, R, W, S, A> asks<MonoidW, R, W, S, A>(Func<R, A> f)
        where MonoidW : Monoid<W> => 
        MRWS<MonoidW, R, W, S, A>.Return(input => f(input.Env));
    ...
    public static RWS<MonoidW, R, W, S, A> local<MonoidW, R, W, S, A>(RWS<MonoidW, R, W, S, A> ma, Func<R, R> f)
        where MonoidW : Monoid<W> => 
        MRWS<MonoidW, R, W, S, A>.Local(ma, f);
    ...
    /// <returns>RWS monad with the mapped environment in as the bound value</returns>
    [Pure]
    public static RWS<MonoidW, R, W, S, Option<A>> choose<MonoidW, R, W, S, A>(
        params RWS<MonoidW, R, W, S, Option<A>>[] monads)
        where MonoidW : Monoid<W> =>
        (env, state) =>
        {
            foreach (var monad in monads)
            {
                ...
                if (res is { IsSucc: true, Value.Value.IsSome: true })
                {
                    return res;
                }
            }
    ...
    [Pure]
    public static RWS<MonoidW, R, W, S, int>  sum<MonoidW, R, W, S>(RWS<MonoidW, R, W, S, int> self)
        where MonoidW : Monoid<W> =>
        self.Sum();
    ...
    [Pure]
    public static RWS<MonoidW, R, W, S, R> fold<MonoidW, R, W, S, A>(RWS<MonoidW, R, W, S, A> self, Func<R, A, R> f)
        where MonoidW : Monoid<W> =>
        self.Fold(f);
    ...
    public static RWS<MonoidW, R, W, S, Unit> tell<MonoidW, R, W, S, A>(W what)
        where MonoidW : Monoid<W> =>
        MRWS<MonoidW, R, W, S, A>.Tell(what);

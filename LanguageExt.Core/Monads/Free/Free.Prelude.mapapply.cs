...
public static partial class Prelude
{
    ...
    /// <param name="f">Mapping function</param>
    /// <returns>Mapped functor</returns>
    public static Free<F, B> map<F, A, B>(Func<A, B> f, Free<F, A> ma) 
        where F : Functor<F> =>
        ma.Map(f);    
    ...
    /// </summary>
    public static Free<F, B> action<F, A, B>(Free<F, A> ma, K<Free<F>, B> mb) 
        where F : Functor<F> =>
    ...
    public static Free<F, B> apply<F, A, B>(Free<F, Func<A, B>> mf, K<Free<F>, A> ma)
        where F : Functor<F> =>
        mf.Kind().Apply(ma).As();

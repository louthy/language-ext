...
public static partial class Prelude
{
    ...
    /// <param name="f">Mapping function</param>
    /// <returns>Mapped functor</returns>
    public static ValidationT<F, M, B> map<F, M, A, B>(Func<A, B> f, ValidationT<F, M, A> ma) 
        where F : Monoid<F> 
        where M : Monad<M> =>
        ma.Map(f);    
    ...
    /// </summary>
    public static ValidationT<F, M, B> action<F, M, A, B>(ValidationT<F, M, A> ma, K<ValidationT<F, M>, B> mb)
        where F : Monoid<F> 
        where M : Monad<M> =>
    ...
    public static ValidationT<F, M, B> apply<F, M, A, B>(ValidationT<F, M, Func<A, B>> mf, K<ValidationT<F, M>, A> ma)
        where F : Monoid<F> 
        where M : Monad<M> =>
        mf.Kind().Apply(ma).As();

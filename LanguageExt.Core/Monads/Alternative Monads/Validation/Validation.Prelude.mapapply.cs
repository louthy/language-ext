...
public static partial class Prelude
{
    ...
    /// <param name="f">Mapping function</param>
    /// <returns>Mapped functor</returns>
    public static Validation<F, B> map<F, A, B>(Func<A, B> f, Validation<F, A> ma) 
        where F : Monoid<F> =>
        ma.Map(f);    
    ...
    /// </summary>
    public static Validation<F, B> action<F, A, B>(Validation<F, A> ma, K<Validation<F>, B> mb) 
        where F : Monoid<F> =>
    ...
    public static Validation<F, B> apply<F, A, B>(Validation<F, Func<A, B>> mf, K<Validation<F>, A> ma)
        where F : Monoid<F> =>
        mf.Kind().Apply(ma).As();

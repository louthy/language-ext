...
public static partial class TryOptionExtensions
{
    ...
    /// <param name="Fail">Delegate to map the exception
    /// <returns>Mapped Try computation</returns>
    [Pure]
    public static TryOption<R> BiMap<A, R>(this TryOption<A> self, Func<A, R> Some, Func<Error, Error> Fail) =>
        Memo(() =>
        {
            var res = self.Try();
            return res switch
            {
                {IsSome: true} x => new OptionalResult<R>(Some(x.Value.Value)),
                {IsNone: true} => OptionalResult<R>.None,
                var x => new OptionalResult<R>(Fail(x.Exception))
            };
        });
    ...
    /// <returns>Mapped Try computation</returns>
    [Pure]
    public static TryOption<R> TriMap<A, R>(this TryOption<A> self, Func<A, R> Some, Func<R> None, Func<Error, Error> Fail) =>
        Memo(() =>
        {
            var res = self.Try();
            return res.IsFaulted
                ? new OptionalResult<R>(Fail(res.Exception))
                : res.Value.IsSome
                    ? Some(res.Value.Value)
                    : None();
        });

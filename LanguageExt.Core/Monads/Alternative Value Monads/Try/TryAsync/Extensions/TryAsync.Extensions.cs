...
public static partial class TryAsyncExtensions
{
    ...
    /// </returns>
    public static TryAsync<A> Do<A>(this TryAsync<A> ma, Action<A> f) => (async () =>
    {
    ...
    [Pure]
    public static TryAsync<A> Filter<A>(this TryAsync<A> self, Func<A, bool> pred) => Memo(async () =>
    {
        var res = await self.Try().ConfigureAwait(false);

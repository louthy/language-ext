...
public static partial class TryExtensions
{
    ...
    /// <param name="Fail">Delegate to map the error</param>
    /// <returns>Mapped Try computation</returns>
    [Pure]
    public static Try<R> BiMap<A, R>(this Try<A> self, Func<A, R> Succ, Func<Error, Error> Fail) => Memo(() =>
    {
        var res = self.Try();
        return res.IsFaulted
            ? new Result<R>(Fail(res.Exception))
            : Succ(res.Value);
    });
    ...
    [Pure]
    public static Try<A> BiFilter<A>(this Try<A> self, Func<A, bool> Succ, Func<Exception, bool> Fail) => Memo(() =>
    {
        var res = self.Try();
        return res.IsFaulted
            ? Fail(res.Exception)

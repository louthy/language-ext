...
public static partial class OptionAsyncExtensions
{
    ...
    /// </summary>
    [Pure]
    public static async ValueTask<IEnumerable<A>> Somes<A>(this IEnumerable<OptionAsync<A>> self)
    {
        var res = await self.Map(o => o.GetData())
                            .WindowMap(identity)
    ...
    public static OptionAsync<A> Add<NUM, A>(this OptionAsync<A> x, OptionAsync<A> y) where NUM : struct, Num<A> =>
        SomeAsync<Func<A, A, A>>(plus<NUM, A>).Apply(x).Apply(y);
    ...
    public static OptionAsync<A> Subtract<NUM, A>(this OptionAsync<A> x, OptionAsync<A> y) where NUM : struct, Num<A> =>
        SomeAsync<Func<A, A, A>>(subtract<NUM, A>).Apply(x).Apply(y);
    ...
    public static OptionAsync<A> Product<NUM, A>(this OptionAsync<A> x, OptionAsync<A> y) where NUM : struct, Num<A> =>
        SomeAsync<Func<A, A, A>>(product<NUM, A>).Apply(x).Apply(y);
    ...
    public static OptionAsync<A> Divide<NUM, A>(this OptionAsync<A> x, OptionAsync<A> y) where NUM : struct, Num<A> =>
        SomeAsync<Func<A, A, A>>(divide<NUM, A>).Apply(x).Apply(y);
    ...
    [Pure]
    public static ValueTask<IEnumerable<R>> Match<T, R>(this IEnumerable<OptionAsync<T>> list,
    ...
    public static ValueTask<int> Sum(this OptionAsync<int> a) =>
        a.IfNone(0);

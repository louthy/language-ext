    ...
    IComparable
{
    ...
    OptionUnsafe(SerializationInfo info, StreamingContext _)
    {
        isSome = (bool)(info.GetValue("IsSome", typeof(bool)) ?? false);
        if (isSome)
        {
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator OptionUnsafe<A>(OptionNone _) =>
        default;
    ...
    public OptionUnsafe<D> Join<B, C, D>(
        ...
        Func<A?, B?, D?> project) =>
        join<EqDefault<C?>, MOptionUnsafe<A>, MOptionUnsafe<B>, MOptionUnsafe<D>, OptionUnsafe<A>, OptionUnsafe<B>, OptionUnsafe<D>, A?, B?,
            C?, D?>(
            this,
            inner,
            outerKeyMap,

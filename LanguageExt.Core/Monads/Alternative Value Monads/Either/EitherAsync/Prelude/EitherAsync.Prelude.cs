...
namespace LanguageExt
{
    public static partial class Prelude
    {
        ...
        [Pure]
        public static EitherAsync<L, R> plus<NUM, L, R>(EitherAsync<L, R> x, EitherAsync<L, R> y)
            where NUM : struct, Num<R> =>
            RightAsync<L, Func<R, R, R>>(default(NUM).Plus).Apply(x).Apply(y);
        ...
        public static EitherAsync<L, R> subtract<NUM, L, R>(EitherAsync<L, R> x, EitherAsync<L, R> y) where NUM : struct, Num<R> =>
            RightAsync<L, Func<R, R, R>>(default(NUM).Subtract).Apply(x).Apply(y);
        ...
        public static EitherAsync<L, R> product<NUM, L, R>(EitherAsync<L, R> x, EitherAsync<L, R> y) where NUM : struct, Num<R> =>
            RightAsync<L, Func<R, R, R>>(default(NUM).Product).Apply(x).Apply(y);
        ...
        public static EitherAsync<L, R> divide<NUM, L, R>(EitherAsync<L, R> x, EitherAsync<L, R> y) where NUM : struct, Num<R> =>
            RightAsync<L, Func<R, R, R>>(default(NUM).Divide).Apply(x).Apply(y);
        ...
        public async static Task<IEnumerable<Ret>> match<L, R, Ret>(IEnumerable<EitherAsync<L, R>> list,
            Func<R, Ret> Right,
            Func<L, Ret> Left) =>
            (await WaitAsync.All(list.Map(item => item.Match(Right, Left))).ConfigureAwait(false)).AsEnumerable();

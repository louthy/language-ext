    ...
    where RT : HasIO<RT, Error>
{
    ...
    public static implicit operator Aff<RT, A>(Transducer<RT, Sum<Error, A>> t) =>
        t.ToAff();
        
    /// <summary>
    /// Convert `Aff` to a `Transducer`
    /// </summary>
    public Transducer<RT, Sum<Error, A>> ToTransducer()
    {
        var self = this;
        return liftAsync(async (RT rt) => (await self.Run(rt).ConfigureAwait(false)).ToSum());
    }
}

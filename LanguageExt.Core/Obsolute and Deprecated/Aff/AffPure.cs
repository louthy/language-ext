...
public readonly struct Aff<A>
{
    ...
    public static implicit operator Aff<A>(Transducer<Unit, Sum<Error, A>> t) =>
        t.ToAff();

    public static implicit operator Transducer<Unit, Sum<Error, A>>(Aff<A> ma) =>
        ma.ToTransducer();

    /// <summary>
    /// Convert `Aff` to a `Transducer`
    /// </summary>
    public Transducer<Unit, Sum<Error, A>> ToTransducer()
    {
        var self = this;
        return liftAsync(async (Unit _) => (await self.Run().ConfigureAwait(false)).ToSum());
    }
}

...
public readonly struct EitherAsync<L, R> :
    ...
    IEitherAsync
{
    public static readonly EitherAsync<L, R> Bottom = new EitherAsync<L, R>();
    internal readonly Task<Either<L, R>> data;

    internal EitherAsync(Task<Either<L, R>> data) =>
        this.data = data;

    internal Task<Either<L, R>> Data =>
        data ?? Either<L, R>.Bottom.AsTask();
    ...
    async ValueTask<object?> GetCase()
    {
        ...
        return data.State switch
               {
                   EitherStatus.IsRight => data.RightValue,
                   EitherStatus.IsLeft  => data.LeftValue,
                   _                    => null
               };
    }
    ...
    public TaskAwaiter<Either<L, R>> GetAwaiter() =>
        Data.Map(d => d.State switch
                      {
                          EitherStatus.IsRight => Either<L, R>.Right(d.RightValue),
                          EitherStatus.IsLeft  => Either<L, R>.Left(d.LeftValue),
                          _                    => Either<L, R>.Bottom
                      }).GetAwaiter();
        ...
        where EqR : Eq<R>
    {
        ...
        return a.State == b.State                   &&
               EqL.Equals(a.LeftValue, b.LeftValue) &&
               EqR.Equals(a.RightValue, b.RightValue);
    }
        ...
        where OrdR : Ord<R>
    {
        var a = await Data.ConfigureAwait(false);
        var b = await rhs.Data.ConfigureAwait(false);
        var c = OrdInt.Compare((int)a.State, (int)b.State);
        if (c != 0) return c;
        c = OrdL.Compare(a.LeftValue, b.LeftValue);
        if (c != 0) return c;
        return OrdR.Compare(a.RightValue, b.RightValue);
    }
        ...
        await IsBottom.ConfigureAwait(false)
            ...
            : await IsLeft.ConfigureAwait(false)
                ? Left == null
                      ...
                      : Left((await Data.ConfigureAwait(false)).LeftValue)
                : Right == null
                    ? throw new ArgumentNullException(nameof(Right))
                    : Right((await Data.ConfigureAwait(false)).RightValue);
        ...
        await IsBottom.ConfigureAwait(false)
            ...
            : await IsLeft.ConfigureAwait(false)
                ...
                : RightAsync == null
                    ...
                    : await RightAsync((await Data.ConfigureAwait(false)).RightValue);
        ...
        await IsBottom.ConfigureAwait(false)
            ...
            : await IsLeft.ConfigureAwait(false)
                ? LeftAsync == null
                      ...
                      : await LeftAsync((await Data.ConfigureAwait(false)).LeftValue)
                : RightAsync == null
                    ? throw new ArgumentNullException(nameof(RightAsync))
                    : await RightAsync((await Data.ConfigureAwait(false)).RightValue);
    ...
    public async Task<Unit> Match(Action<R> Right, Action<L> Left, Action? Bottom = null)
    {
        ...
        {
            Right.Invoke((await Data.ConfigureAwait(false)).RightValue);
        }
        else if (await IsLeft.ConfigureAwait(false))
        {
            Left.Invoke((await Data.ConfigureAwait(false)).LeftValue);
        }
    ...
    public async Task<Unit> MatchAsync(Action<R> Right, Func<L, Task> LeftAsync, Action? Bottom = null)
    {
        ...
        {
            await LeftAsync((await Data.ConfigureAwait(false)).LeftValue).ConfigureAwait(false);
        }
        else if (await IsBottom.ConfigureAwait(false))
        {
            if (Bottom != null)
    ...
    public async Task<Unit> MatchAsync(Func<R, Task> RightAsync, Action<L> Left, Action? Bottom = null)
    {
        if (await IsRight.ConfigureAwait(false))
        {
            await RightAsync((await Data.ConfigureAwait(false)).RightValue).ConfigureAwait(false);
        }
        else if (await IsLeft)
        {
            Left?.Invoke((await Data).LeftValue);
        }
        else if (await IsBottom.ConfigureAwait(false))
        {
            if (Bottom != null)
    ...
    public TryOptionAsync<R> ToTryOption() =>
        Match(
            ...
            Left: _ => TryOption(Option<R>.None),
    ...
    public static EitherAsync<L, R> Right(R value) =>
        isnull(value)
            ? throw new ValueIsNullException()
            : new EitherAsync<L, R>(Either<L, R>.Right(value).AsTask());

    [Pure]
    public static EitherAsync<L, R> RightAsync(Task<R> value) =>
        isnull(value)
            ? throw new ArgumentNullException(nameof(value))
            : new EitherAsync<L, R>(value.Map(Either<L, R>.Right));

    [Pure]
    public static EitherAsync<L, R> Left(L value) =>
        isnull(value)
            ? throw new ValueIsNullException()
            : new EitherAsync<L, R>(Either<L, R>.Left(value).AsTask());

    [Pure]
    public static EitherAsync<L, R> LeftAsync(Task<L> value) =>
        isnull(value)
            ? throw new ArgumentNullException(nameof(value))
            : new EitherAsync<L, R>(value.Map(Either<L, R>.Left));

    [Pure]
    internal async Task<R> RightValue() =>
        Check.NullReturn(
            await IsRight.ConfigureAwait(false)
                ? (await Data.ConfigureAwait(false)).RightValue
    ...
    internal async Task<L> LeftValue() =>
        Check.NullReturn(
            await IsLeft.ConfigureAwait(false)
                ? (await Data.ConfigureAwait(false)).LeftValue
    ...
    public EitherAsync<R, L> Swap()
    {
        ...
        async Task<Either<R, L>> Go(Task<Either<L, R>> self)
        {
            var data = await self.ConfigureAwait(false);

            return data.State switch
                   {
                       EitherStatus.IsRight => Either<R, L>.Left(data.RightValue),
                       EitherStatus.IsLeft  => Either<R, L>.Right(data.LeftValue),
                       _                    => Either<R, L>.Bottom
                   };
        }
    }
    ...
    {
        async Task<Either<L2, R2>> Do(EitherAsync<L, R> self, Func<R, R2> right, Func<L, L2> left) =>
            (await self.IsRight.ConfigureAwait(false))
                ? Either<L2, R2>.Right(right(await self.RightValue().ConfigureAwait(false)))
                : await self.IsLeft.ConfigureAwait(false)
                    ? Either<L2, R2>.Left(left(await self.LeftValue().ConfigureAwait(false)))
                    : Either<L2, R2>.Bottom;
    ...
    {
        async Task<Either<L2, R2>> Do(EitherAsync<L, R> self, Func<R, Task<R2>> right, Func<L, L2> left) =>
            (await self.IsRight.ConfigureAwait(false))
                ? Either<L2, R2>.Right(await right(await self.RightValue().ConfigureAwait(false)).ConfigureAwait(false))
                : await self.IsLeft.ConfigureAwait(false)
                    ? Either<L2, R2>.Left(left(await self.LeftValue().ConfigureAwait(false)))
                    : Either<L2, R2>.Bottom;
    ...
    {
        async Task<Either<L2, R2>> Do(EitherAsync<L, R> self, Func<R, R2> right, Func<L, Task<L2>> left) =>
            await self.IsRight.ConfigureAwait(false)
                ? Either<L2, R2>.Right(right(await self.RightValue().ConfigureAwait(false)))
                : await self.IsLeft.ConfigureAwait(false)
                    ? Either<L2, R2>.Left(await left(await self.LeftValue().ConfigureAwait(false)).ConfigureAwait(false))
                    : Either<L2, R2>.Bottom;
    ...
    {
        async Task<Either<L2, R2>> Do(EitherAsync<L, R> self, Func<R, Task<R2>> right, Func<L, Task<L2>> left) =>
            await self.IsRight.ConfigureAwait(false)
                ? Either<L2, R2>.Right(await right(await self.RightValue().ConfigureAwait(false)).ConfigureAwait(false))
                : await self.IsLeft.ConfigureAwait(false)
                    ? Either<L2, R2>.Left(await left(await self.LeftValue()).ConfigureAwait(false))
                    : Either<L2, R2>.Bottom;
    ...
    {
        async Task<Either<L, Ret>> Do(EitherAsync<L, R> self, Func<R, EitherAsync<L, Ret>> ff)
        {
            ...
            else
            {
                return (await self.IsLeft.ConfigureAwait(false))
                           ? Either<L, Ret>.Left(await self.LeftValue().ConfigureAwait(false))
                           : Either<L, Ret>.Bottom;
            }
        }
    ...
    {
        async Task<Either<L, Ret>> Do(EitherAsync<L, R> self, Func<R, Task<EitherAsync<L, Ret>>> ff)
        {
            if (await self.IsRight.ConfigureAwait(false))
            {
    ...
    {
        async Task<Either<L, B>> Do(EitherAsync<L, R> self, Func<R, EitherAsync<L, B>> right, Func<L, EitherAsync<L, B>> left)
        {
            if (await self.IsRight.ConfigureAwait(false))
            {
    ...
    {
        async Task<Either<L, B>> Do(EitherAsync<L, R> self, Func<R, Task<EitherAsync<L, B>>> right, Func<L, EitherAsync<L, B>> left)
        {
            if (await self.IsRight.ConfigureAwait(false))
            {
    ...
    {
        async Task<Either<L, B>> Do(EitherAsync<L, R> self, Func<R, EitherAsync<L, B>> right, Func<L, Task<EitherAsync<L, B>>> left)
        {
            if (await self.IsRight.ConfigureAwait(false))
            {
                var mb = right(await self.RightValue().ConfigureAwait(false));
    ...
    {
        async Task<Either<L, B>> Do(EitherAsync<L, R> self, Func<R, Task<EitherAsync<L, B>>> right, Func<L, Task<EitherAsync<L, B>>> left)
        {
            if (await self.IsRight.ConfigureAwait(false))
            {
    ...
    {
        async Task<Either<B, R>> Do(EitherAsync<L, R> self)
        {
            if (await self.IsRight.ConfigureAwait(false))
            {
                return Either<B, R>.Right(await self.RightValue().ConfigureAwait(false));
            }
            if (await self.IsLeft.ConfigureAwait(false))
            {
                var mb = Left(await self.LeftValue().ConfigureAwait(false));
...
/// </summary>
[Obsolete(Change.UseEffMonadInstead)]
public readonly struct EitherAsyncContext<L, R, Ret>
{
...
/// </summary>
[Obsolete(Change.UseEffMonadInstead)]
public readonly struct EitherAsyncUnitContext<L, R>
{
    readonly EitherAsync<L, R> either;

...
{
    internal class SeqStrictShared<A> : ISeqInternal<A>
    {
        ...
        public SeqStrictShared(A[] data, ArrayPool<A> pool, int start, int count, int consDisallowed, int addDisallowed)
        {
            this.data           = data;
            this.pool           = pool;
            this.start          = start;
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public SeqStrictShared(A[] data, ArrayPool<A> pool, int start, int count)
        {
            this.data           = data;
            this.pool           = pool;
            this.start          = start;
        ...
        /// <summary>
        /// Release the data when we're not referenced any more
        /// </summary>
        ~SeqStrictShared() =>
            pool.Return(data);
        ...
        public ISeqInternal<A> Add(A value)
        {
            ...
            {
                data[end] = value;
                return new SeqStrictShared<A>(data, pool, start, count + 1);
            }
        }
        ...
        public ISeqInternal<A> Cons(A value)
        {
            ...
            else
            {
                var nstart = start - 1;
                data[nstart] = value;
                return new SeqStrictShared<A>(data, pool, start - 1, count + 1, 0, NoAdd);
            }
        }

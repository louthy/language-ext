...
namespace LanguageExt
{
    ...
    public static class Patch
    {
        ...
        {
            (Seq<Edit<EqA, A>.Insert> inserts, Seq<Edit<EqA, A>.Delete> deletes, Seq<Edit<EqA, A>.Replace> replaces) partition3(Seq<Edit<EqA, A>> grp)
            {
        ...
        public static bool applicable<EqA, A>(Patch<EqA, A> pa, IEnumerable<A> va) where EqA : struct, Eq<A>
        {
            ...
            return pa.Edits.ForAll(e =>
                e is Edit<EqA, A>.Insert ins ? ins.Position <= i.Count
              : e is Edit<EqA, A>.Delete del ? i.Elem(del.Position).Match(
                                                     Some: ci => default(EqA).Equals(del.Element, ci),
                                                     None: () => false)
              : e is Edit<EqA, A>.Replace repl ? i.Elem(repl.Position).Match(
        ...
        {
            bool go(Seq<Edit<EqA, A>> ea, Seq<Edit<EqA, A>> eb, int off)
            {
                ...
                else
                {
                    ...
                    {
                        return default(EqA).Equals(replA1.ReplaceElement, replB1.Element) && go(xs, ys, off);
                    }
                    else if (x is Edit<EqA, A>.Replace replA2 && yi is Edit<EqA, A>.Delete del3)
                    {
                        return default(EqA).Equals(replA2.ReplaceElement, del3.Element) && go(xs, ys, off);
                    }
                    else if (x is Edit<EqA, A>.Insert ins3 && yi is Edit<EqA, A>.Replace replB2)
                    {
                        return default(EqA).Equals(ins3.Element, replB2.Element) && go(xs, ys, off + offset(x));
                    }
                    else if (x is Edit<EqA, A>.Insert ins4 && yi is Edit<EqA, A>.Delete del4)
                    {
                        return default(EqA).Equals(ins4.Element, del4.Element) && go(xs, ys, off + offset(x));
                    }
                    else
                    {
                        throw new NotSupportedException();
        ...
        public static int sizeChange<EqA, A>(Patch<EqA, A> patch) where EqA : struct, Eq<A>
        {
            ...
            {
                if (item is Edit<EqA, A>.Delete) count--;
                if (item is Edit<EqA, A>.Insert) count++;
            }
            return count;
        }
        ...
        public static IEnumerable<A> apply<EqA, A>(Patch<EqA, A> patch, IEnumerable<A> va) where EqA : struct, Eq<A>
        {
            ...
            Unit go(Seq<Edit<EqA, A>> edits, SpanArray<A> src, SpanArray<A> dest, int si)
            {
                if (edits.IsEmpty)
                {
                    return dest.Count > 0
                ...
                else
                {
                    ...
                    if (x > 0)
                    {
                        ...
                        return go(edits, src.Skip(x), dest.Skip(x), si + x);
                    }
                    else
                    {
                        switch (edits.Head)
                        {
                            case Edit<EqA, A>.Insert insert:
                                ...
                                return go(edits.Tail, src, dest.Tail, si);

                            case Edit<EqA, A>.Delete delete:
                                return go(edits.Tail, src.Tail, dest, si + 1);

                            case Edit<EqA, A>.Replace replace:
                                dest[0] = replace.ReplaceElement;
                                return go(edits.Tail, src.Tail, dest.Tail, si + 1);
        ...
        {
            (Seq<Edit<EqA, A>> a, Seq<Edit<EqA, A>> b) go(Seq<Edit<EqA, A>> xs, int a, Seq<Edit<EqA, A>> ys, int b)
            {
                ...
                if (ord < 0)
                {
                    var (_1, _2) = go(xs.Tail, a, ys, b + offset(x));
                    return (x.Index(i => i + a).Cons(_1), _2);
                }
                else if (ord > 0)
                {
                    var (_1, _2) = go(xs, a + offset(y), ys.Tail, b);
                ...
                else
                {
                    if (x == y)
                    {
                        return go(xs.Tail, a + offset(y), ys.Tail, b + offset(x));
                    }
                    else if (x is Edit<EqA, A>.Insert ins1 && y is Edit<EqA, A>.Insert ins2)
                    {
                        ...
                        return cons2(
                            ...
                            go(xs.Tail, a + offset(y), ys.Tail, b + offset(x)));
                    }
                    else if (x is Edit<EqA, A>.Replace repl1 && y is Edit<EqA, A>.Replace repl2)
                    {
                        ...
                        return cons2(
                            ...
                            go(xs.Tail, a, ys.Tail, b));
                    }
                    else if (x is Edit<EqA, A>.Insert ins3)
                    {
                        var (_1, _2) = go(xs.Tail, a, ys, b + offset(x));
                        return (x.Index(i => i + a).Cons(_1), _2);
                    }
                    else if (y is Edit<EqA, A>.Insert ins4)
                    {
                        var (_1, _2) = go(xs, a + offset(y), ys.Tail, b);
                        return (_1, y.Index(i => i + b).Cons(_2));
                    }
                    else if (x is Edit<EqA, A>.Replace repl3 && y is Edit<EqA, A>.Delete del1)
                    {
                        var (_1, _2) = go(xs.Tail, a + offset(y), ys.Tail, b);
                        return (_1, (Edit<EqA, A>.Delete.New(repl3.Position, repl3.ReplaceElement)).Index(i => i + b).Cons(_2));
                    }
                    else if (x is Edit<EqA, A>.Delete del3 && y is Edit<EqA, A>.Replace repl4)
                    {
                        var (_1, _2) = go(xs.Tail, a, ys.Tail, b + offset(x));
                        return ((Edit<EqA, A>.Delete.New(repl4.Position, repl4.ReplaceElement)).Index(i => i + a).Cons(_1), _2);
                    }
                    else if (x is Edit<EqA, A>.Delete del4 && y is Edit<EqA, A>.Delete del5)
                    {
                        return go(xs.Tail, a + offset(y), ys.Tail, b + offset(x));
                    }
                    else
            ...
            (Seq<Edit<EqA, A>>, Seq<Edit<EqA, A>>) cons2((Edit<EqA, A> cx, Edit<EqA, A> cy) head, (Seq<Edit<EqA, A>> cxs, Seq<Edit<EqA, A>> cys) tail) =>
                (head.cx.Cons(tail.cxs), head.cy.Cons(tail.cys));

            var (pi, qi) = go(p.Edits, 0, q.Edits, 0);
        ...
        public static Patch<EqA, A> diff<EqA, A>(IEnumerable<A> va, IEnumerable<A> vb) where EqA : struct, Eq<A>
        {
            ...
            Seq<Edit<EqA, A>> adjust(int o, Seq<Edit<EqA, A>> list) =>
                list.IsEmpty
                    ? Seq<Edit<EqA, A>>()

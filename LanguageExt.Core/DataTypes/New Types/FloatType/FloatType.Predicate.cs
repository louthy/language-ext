    ...
    where SELF : FloatType<SELF, FLOATING, A, PRED>
{
    ...
    {
        Value = (A?)info.GetValue("Value", typeof(A)) ?? throw new SerializationException();
        if (!PRED.True(Value)) throw new ArgumentOutOfRangeException(nameof(Value));
        if (isnull(Value)) throw new ArgumentNullException(nameof(Value));
    }
    ...
    public virtual int CompareTo(SELF? other) =>
        ReferenceEquals(other, null)
            ? 1
            : FLOATING.Compare(Value, other.Value);
    ...
    public virtual bool Equals(SELF? other) =>
        !ReferenceEquals(other, null) && FLOATING.Equals(Value, other.Value);
    ...
    public static SELF operator -(FloatType<SELF, FLOATING, A, PRED> x) =>
        New(FLOATING.Subtract(FLOATING.FromInteger(0), ValueOrDefault(x)));
    ...
    [Pure]
    public static SELF operator -(FloatType<SELF, FLOATING, A, PRED> lhs, FloatType<SELF, FLOATING, A, PRED> rhs) =>
        New(FLOATING.Subtract(ValueOrDefault(lhs), ValueOrDefault(rhs)));
    ...
    [Pure]
    public static SELF operator /(FloatType<SELF, FLOATING, A, PRED> lhs, FloatType<SELF, FLOATING, A, PRED> rhs) =>
        New(FLOATING.Divide(ValueOrDefault(lhs), ValueOrDefault(rhs)));

    [Pure]
    public static bool operator ==(FloatType<SELF, FLOATING, A, PRED> lhs, FloatType<SELF, FLOATING, A, PRED> rhs) =>
        FLOATING.Equals(ValueOrDefault(lhs), ValueOrDefault(rhs));
    ...
    [Pure]
    public static bool operator >(FloatType<SELF, FLOATING, A, PRED> lhs, FloatType<SELF, FLOATING, A, PRED> rhs) =>
        FLOATING.Compare(ValueOrDefault(lhs), ValueOrDefault(rhs)) > 0;
    ...
    public static SELF FromInteger(int value) =>
        New(FLOATING.FromInteger(value));
}

    ...
    Foldable<MA, A>
{
    ...
    public static abstract MA Return(A x);
}
    ...
    Foldable<Env, MA, A>
{
    ...
    [Pure]
    public static abstract MB Bind<MONADB, MB, B>(MA ma, Func<A, MB> f) 
    ...
    ///
    ///     MA monad = MonadA.Return(a);
    ///  
    /// To construct a monad of type `MA`.  
    /// </remarks>
    /// <param name="x">Value to bind</param>
    /// <returns>Monad of type `MA`</returns>
    public static abstract MA Return(Func<Env, A> f);
    ...
    ///         where MONADB : Monad<Unit, (W, bool), MB, B> =>
    ///             MONADB.Id(_ =>
    ///             {
    ...
    public static abstract MA BindReturn(Out outputma, MA mb);
    ...
    ///         where MONADB : Monad<S, (S State, bool IsFaulted), MB, B> =>
    ///             MONADB.Run(state =>
    ///             {
    ...
    /// </remarks>
    public static abstract MA Run(Func<Env, MA> ma);

    /// <summary>
    /// Produce a monad of `MA` in it's failed state
    /// </summary>
    [Pure]
    public static abstract MA Fail(object? err = null);

    /// <summary>
    /// Associative binary operation
    /// </summary>
    [Pure]
    public static abstract MA Plus(MA ma, MA mb);

    /// <summary>
    /// Neutral element (`None` in `Option` for example)
    /// </summary>
    [Pure]
    public static abstract MA Zero();

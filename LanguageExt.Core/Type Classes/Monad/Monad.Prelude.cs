...
public static partial class TypeClass
{
    ...
    [Pure]
    public static MA Return<MONAD, MA, A>(A x) where MONAD : Monad<MA, A> =>
        MONAD.Return(_ => x);

    [Pure]
    public static MB traverse<MonadA, MonadB, MA, MB, A, B>(MA ma, Func<A, B> f)
        where MonadA : Monad<Unit, Unit, MA, A>
        where MonadB : Monad<Unit, Unit, MB, B> =>
    ...
    public static Func<Env, MB> traverse<Env, Out, MonadA, MonadB, MA, MB, A, B>(MA ma, Func<A, B> f)
        where MonadA : Monad<Env, Out, MA, A>
        where MonadB : Monad<Env, Out, MB, B> =>
        MonadA.Fold(ma, MonadB.Zero(), (s, a) => MonadB.Plus(s, MonadB.Return(_ => f(a))));

    [Pure]
    public static Func<Env, MB> traverse<Env, Out, MonadA, MonadB, MA, MB, A, B>(MA ma, Func<A, MB> f)
        where MonadA : Monad<Env, Out, MA, A>
        where MonadB : Monad<Env, Out, MB, B> =>
        MonadA.Fold(ma, MonadB.Zero(), (s, a) => MonadB.Plus(s, f(a)));
    ...
    public static MB bind<MONADA, MONADB, MA, MB, A, B>(MA ma, Func<A, MB> f)
        ...
        where MONADB : Monad<MB, B> =>
        MONADA.Bind<MONADB, MB, B>(ma, f);

    /// <summary>
    /// Produce a failure value
    /// </summary>
    [Pure]
    public static MA fail<MONAD, MA, A>(Exception? err = null) where MONAD : Monad<MA, A> =>
        MONAD.Fail(err);
    ...
    [Pure]
    public static MB liftM<FUNCTOR, MA, MB, A, B>(MA ma, Func<A, B> f)
        where FUNCTOR : Functor<MA, MB, A, B> =>
        FUNCTOR.Map(ma, f);
    ...
    public static MD join<EQ, MONADA, MONADB, MONADD, MA, MB, MD, A, B, C, D>(
        ...
        where MONADA : Monad<MA, A>
        where MONADB : Monad<MB, B>
        where MONADD : Monad<MD, D> =>
        MONADA.Bind<MONADD, MD, D>(self,  x =>
                                       MONADB.Bind<MONADD, MD, D>(inner, y =>
                                                                      EQ.Equals(outerKeyMap(x), innerKeyMap(y))
                                                                          ? MONADD.Return(_ => project(x, y))
                                                                          : MONADD.Fail()));
    ...
    public static MC SelectMany<MONADA, MONADB, MONADC, MA, MB, MC, A, B, C>(
        ...
        where MONADA : Monad<MA, A>
        where MONADB : Monad<MB, B>
        where MONADC : Monad<MC, C> =>
        MONADA.Bind<MONADC, MC, C>( self,    t => 
                                        MONADB.Bind<MONADC, MC, C>( bind(t), u => 
                                                                        MONADC.Return(project(t, u))));
    ...
    public static IEnumerable<C> SelectMany<MONADA, MA, A, B, C>(
        ...
        Func<A, B, C> project)
        where MONADA : Monad<MA, A> =>
        MONADA.Bind<MEnumerable<C>, IEnumerable<C>, C>(self, a =>
    ...
    public static Seq<C> SelectMany<MONADA, MA, A, B, C>(
        ...
        Func<A, B, C> project)
        where MONADA : Monad<MA, A> =>
        MONADA.Bind<MSeq<C>, Seq<C>, C>(self, a =>
                                            bind(a).Select(b => project(a, b)));
    ...
    [Pure]
    public static MA mzero<MPLUS, MA, A>() where MPLUS : Monad<MA, A> =>
        MPLUS.Zero();
    ...
    [Pure]
    public static MA mplus<MPLUS, MA, A>(MA x, MA y) where MPLUS : Monad<MA, A> =>
        MPLUS.Plus(x, y);
    ...
    [Pure]
    public static MA msum<MPLUS, MA, A>(params MA[] xs) where MPLUS : Monad<MA, A> =>
    ...
    /// <returns>The summed monads</returns>
    [Pure]
    public static MA msum<MPLUS, MA, A>(IEnumerable<MA> xs) where MPLUS : Monad<MA, A> =>
        xs.Fold(mzero<MPLUS, MA, A>(), (s, x) => mplus<MPLUS, MA, A>(s, x));
    ...
    [Pure]
    public static MA filter<MPLUS, MA, A>(MA ma, Func<A, bool> pred) where MPLUS : Monad<MA, A> =>
        MPLUS.Bind<MPLUS, MA, A>(ma,
                                 x => pred(x)
                                          ? ma
                                          : mzero<MPLUS, MA, A>());
}

...
namespace LanguageExt.Interfaces
{
    public interface AtomIO
    {
        ...
        Ref<A> Ref<A>(A value, Func<A, bool> validator = null);

        /// <summary>
        /// Run the op within a new transaction
        /// If a transaction is already running, then this becomes part of the parent transaction
        /// </summary>
        R Sync<R>(Func<R> op, Isolation isolation = Isolation.Snapshot);

        /// <summary>
        /// Run the op within a new transaction
        /// If a transaction is already running, then this becomes part of the parent transaction
        /// </summary>
        Unit Sync(Action op, Isolation isolation = Isolation.Snapshot);
        ...
        /// <param name="f">Function to update the `Ref`</param>
        /// <returns>The value returned from `f`</returns>
        A Swap<A, X>(Ref<A> r, X x, Func<X, A, A> f);
        ...
        /// <param name="f">Function to update the `Ref`</param>
        /// <returns>The value returned from `f`</returns>
        A Swap<A, X, Y>(Ref<A> r, X x, Y y, Func<X, Y, A, A> f);
        ...
        /// </summary>
        A Commute<A>(Ref<A> r, Func<A, A> f);
        ...
        /// Commute allows for more concurrency than just setting the Ref's value
        /// </summary>
        A Commute<A, X>(Ref<A> r, X x, Func<X, A, A> f);
        ...
        /// Commute allows for more concurrency than just setting the Ref's value
        /// </summary>
        A Commute<A, X, Y>(Ref<A> r, X x, Y y, Func<X, Y, A, A> f);
        ...
        /// coordinated with any other, and for which you wish to make synchronous changes.
        /// </remarks>
        Option<Atom<A>> Atom<A>(A value, Func<A, bool> validator);
        ...
        /// coordinated with any other, and for which you wish to make synchronous changes.
        /// </remarks>
        Atom<M, A> Atom<M, A>(M metadata, A value);
        ...
        /// coordinated with any other, and for which you wish to make synchronous changes.
        /// </remarks>
        Option<Atom<M, A>> Atom<M, A>(M metadata, A value, Func<A, bool> validator);
        ...
        bool Swap<A>(Atom<A> atom, Func<A, A> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        Task<bool> SwapAsync<A>(Atom<A> atom, Func<A, Task<A>> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        bool Swap<M, A>(Atom<M, A> atom, Func<M, A, A> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        Task<bool> SwapAsync<M, A>(Atom<M, A> atom, Func<M, A, Task<A>> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        bool Swap<X, A>(Atom<A> atom, X x, Func<X, A, A> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        Task<bool> SwapAsync<X, A>(Atom<A> atom, X x, Func<X, A, Task<A>> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        bool Swap<M, X, A>(Atom<M, A> atom, X x, Func<M, X, A, A> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        Task<bool> SwapAsync<M, X, A>(Atom<M, A> atom, X x, Func<M, X, A, Task<A>> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        bool Swap<X, Y, A>(Atom<A> atom, X x, Y y, Func<X, Y, A, A> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        Task<bool> SwapAsync<X, Y, A>(Atom<A> atom, X x, Y y, Func<X, Y, A, Task<A>> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        bool Swap<M, X, Y, A>(Atom<M, A> atom, X x, Y y, Func<M, X, Y, A, A> f);
        ...
        /// <returns>`true` if new-value passes any validation and was successfully set.  `false`
        /// will only be returned if the `validator` fails.</returns>
        ValueTask<bool> SwapAsync<M, X, Y, A>(Atom<M, A> atom, X x, Y y, Func<M, X, Y, A, Task<A>> f);
    }

    /// <summary>
    /// Type-class giving a struct the trait of supporting task atomic operations
    /// </summary>
    /// <typeparam name="RT">Runtime</typeparam>
    [Typeclass("*")]
    public interface HasAtom<RT>

    ...
    public override IO<B> Invoke(EnvIO envIO)
    {
        ...
        if (task.IsCompleted)
        {
            ...
            return Predicate((state, value))
                       ? Next(state).As()
                       : new IOFoldingSync<S, A, B>(Operation, Schedule.Run().GetEnumerator(), state, Folder, Predicate, Next);
        }
        else
        {
...
{
    public override IO<C> Map<C>(Func<B, C> f) => 
        new IOFoldingInitialAsync<S, A, C>(First, Operation, Schedule, State, Folder, Predicate, x => Next(x).Map(f));

    public override IO<C> Bind<C>(Func<B, K<IO, C>> f) => 
        new IOFoldingInitialAsync<S, A, C>(First, Operation, Schedule, State, Folder, Predicate, x => Next(x).Bind(f));
    ...
    public override async ValueTask<IO<B>> Invoke(EnvIO envIO) 
    {
        ...
        if (Schedule.MoveNext())
        {
            ...
            return Predicate((state, value))
                       ? Next(state).As()
                       : new IOFoldingAsync<S, A, B>(Operation, Schedule, state, Folder, Predicate, Next);
        }
        else
        {
            return Next(State).As();
...
{
    public override IO<C> Map<C>(Func<B, C> f) => 
        new IOFoldingAsync<S, A, C>(Operation, Schedule, State, Folder, Predicate, x => Next(x).Map(f));

    public override IO<C> Bind<C>(Func<B, K<IO, C>> f) => 
        new IOFoldingAsync<S, A, C>(Operation, Schedule, State, Folder, Predicate, x => Next(x).Bind(f));
...
{
    public override IO<C> Map<C>(Func<B, C> f) => 
        new IOFoldingSync<S, A, C>(Operation, Schedule, State, Folder, Predicate, x => Next(x).Map(f));

    public override IO<C> Bind<C>(Func<B, K<IO, C>> f) => 
        new IOFoldingSync<S, A, C>(Operation, Schedule, State, Folder, Predicate, x => Next(x).Bind(f));
    ...
    public override IO<B> Invoke(EnvIO envIO) 
    {
        if (Schedule.MoveNext())
        {
            ...
            return Predicate((state, value))
                       ? Next(state).As()
                       : new IOFoldingSync<S, A, B>(Operation, Schedule, state, Folder, Predicate, Next);
        }
        else
        {
            return Next(State).As();

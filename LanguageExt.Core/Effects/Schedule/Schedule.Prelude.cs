...
public static partial class Prelude
{
    ...
    [Pure]
    public static Schedule ToSchedule(this IEnumerable<PositiveDuration> enumerable) => new(enumerable);
    ...
    public static Schedule Intersect(this Schedule a, Schedule b) =>
        a.AsEnumerable().Zip(b.AsEnumerable())
            .Select(t => (PositiveDuration)Math.Max(t.Item1, t.Item2))
            .ToSchedule();
    ...
    {
        IEnumerable<PositiveDuration> Loop()
        {
    ...
    [Pure]
    internal static Eff<S> Run<A, S>(
        this Schedule schedule, Eff<A> effect, S state, Func<S, A, S> fold, Func<Fin<A>, bool> pred) =>
        EffMaybe(() =>
        {
            Fin<A> result;
            ...
            {
                wait.WaitOne((int)enumerator.Current);
                // run again
                RunAndFold();
            }
    ...
    [Pure]
    internal static Eff<A> Run<A>(
        this Schedule schedule, Eff<A> effect, Func<Fin<A>, bool> pred) =>
        schedule.Run(effect, default(A), static (_, __) => _, pred);

    [Pure]
    internal static Eff<RT, S> Run<RT, A, S>(
        this Schedule schedule, Eff<RT, A> effect, S state, Func<S, A, S> fold, Func<Fin<A>, bool> pred)
        where RT : struct =>
        EffMaybe<RT, S>(env =>
        {
            Fin<A> result;
    ...
    [Pure]
    internal static Eff<RT, A> Run<RT, A>(
        this Schedule schedule, Eff<RT, A> effect, Func<Fin<A>, bool> pred) where RT : struct =>
        schedule.Run(effect, default(A), static (_, __) => _, pred);

    [Pure]
    internal static Aff<S> Run<A, S>(
        this Schedule schedule, Aff<A> effect, S state, Func<S, A, S> fold, Func<Fin<A>, bool> pred) =>
        AffMaybe(async () =>
        {
            Fin<A> result;
    ...
    [Pure]
    internal static Aff<A> Run<A>(
        this Schedule schedule, Aff<A> effect, Func<Fin<A>, bool> pred) =>
        schedule.Run(effect, default(A), static (_, __) => _, pred);

    [Pure]
    internal static Aff<RT, S> Run<RT, A, S>(
        this Schedule schedule, Aff<RT, A> effect, S state, Func<S, A, S> fold, Func<Fin<A>, bool> pred)
    ...
    [Pure]
    internal static Aff<RT, A> Run<RT, A>(
        this Schedule schedule, Aff<RT, A> effect, Func<Fin<A>, bool> pred)
        where RT : struct, HasCancel<RT> =>
        schedule.Run(effect, default(A), static (_, __) => _, pred);
}

...
public static partial class Prelude
{
    ...
    /// state/value pair 
    /// </summary>
    public static K<M, S> foldWhileMIO<T, A, M, S>(
        Func<S, A, K<M, S>> f, 
        Func<(S State, A Value), bool> predicate, 
        S initialState, 
        K<T, A> ta) 
    ...
    /// state/value pair 
    /// </summary>
    public static K<M, S> foldUntilMIO<T, A, M, S>(
        Func<S, A, K<M, S>> f, 
        Func<(S State, A Value), bool> predicate, 
        S initialState, 
        K<T, A> ta) 
    ...
    /// list, reduces the list using the binary operator, from right to left.
    /// </summary>
    public static K<M, S> foldMIO<T, A, M, S>(
        Func<S, A, K<M, S>> f, 
        S initialState, 
        K<T, A> ta) 
        where T : FoldableIO<T>
    ...
    /// </summary>
    public static K<F, Unit> iterMIO<T, A, F, B>(Func<A, K<F, B>> f, K<T, A> ta)
        where T : FoldableIO<T>
        where F : MonadIO<F> =>
        ta.IterM(f);

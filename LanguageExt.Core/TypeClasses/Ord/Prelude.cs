...
namespace LanguageExt
{
    public static partial class TypeClass
    {
        ...
        public static int compare<ORD, A>(TryOption<A> x, TryOption<A> y) where ORD : struct, Ord<A>
        {
            var rx = x.Try();
            var ry = x.Try();
            if (rx.IsFaulted && ry.IsFaulted) return 0;
            if (rx.IsFaulted && !ry.IsFaulted) return -1;
            if (!rx.IsFaulted && ry.IsFaulted) return 1;
            return compare<ORD, A>(rx.Value, ry.Value);
        }
        ...
        {
            var cmp = x.State.CompareTo(y);
            if (cmp == 0)
            {
                return x.Match(
                    Right: a =>
                        y.Match(
                            Right: b => compare<ORD, R>(a, b),
                            Left:  _ => 0),
                    Left:  _ => 0);
            }
            else

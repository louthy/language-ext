...
public struct ApplValidation<FAIL, A, B, C> :
    Applicative<Validation<FAIL, Func<A, Func<B, C>>>, Validation<FAIL, Func<B, C>>, Validation<FAIL, A>, Validation<FAIL, B>, Validation<FAIL, C>, A, B>
{
    [Pure]
    public static Validation<FAIL, Func<B, C>> Apply(Validation<FAIL, Func<A, Func<B, C>>> fab, Validation<FAIL, A> fa) =>
        (fab, fa).Apply((f, a) => f(a));

    [Pure]
    public static Validation<FAIL, C> Apply(Validation<FAIL, Func<A, Func<B, C>>> fab, Validation<FAIL, A> fa, Validation<FAIL, B> fb) =>
        (fab, fa, fb).Apply((f, a, b) => f(a)(b));

    [Pure]
    public static Validation<FAIL, A> Pure(A x) =>
        Success<FAIL, A>(x);
}

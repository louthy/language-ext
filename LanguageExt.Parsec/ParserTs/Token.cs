...
namespace LanguageExt.Parsec
{
    ...
    public static class TokenT
    {
        ...
        public static GenTokenParserT makeTokenParser(GenLanguageDefT def)
        {
            ...
            Parser<char, Unit> multiLineComment = null;
            Parser<char, Unit> inCommentMulti = null;
            Parser<char, Unit> inCommentSingle = null;
            ...
            var dec = from x in many1(digit)
                      ...
                      from n in v.Match(
                          Some: d  => result<char,int>(d),
                          None: () => failure<char, int>("Not a valid decimal value"))
                      select n;
            ...
            var octal = (from _ in ch('o')
                         ...
                         from n in v.Match(
                             Some: d  => result<char,int>(d),
                             None: () => failure<char, int>("Not a valid octal value"))
                         select n)
                        .label("octal number");
            ...
            var zeroNumber = (from _ in ch('0')
                              from r in choice(hexadecimal, octal, dec, result<char,int>(0))
                              select r)
                             .label("");
            ...
            var sign = choice(from _ in ch('-')
                              ...
                              from _ in ch('+')
                              select fun((double d) => d),
                              result<char, Func<double,double>>(fun((double d) => d)));

            var signi = choice(from _ in ch('-')
                               select fun((int d) => -d),
                               from _ in ch('+')
                               select fun((int d) => d),
                               result<char, Func<int, int>>(fun((int d) => d)));
            ...
            var fraction = (from _ in ch('.')
                            ...
                            from n in f.Match(
                                Some: d  => result<char,int>(d),
                                None: () => failure<char, int>("Not a valid fractional value"))
                            select n / 10.0)
                           .label("fraction");
            ...
            var zeroNumFloat =
                choice(
                    ...
                    decimalFloat,
                    fractFloat(0),
                    result<char, Either<int,double>>(Left<int, double>(0)));
            ...
            var floating = from si in optionOrElse("", from x in oneOf("+-") select x.ToString())
                           ...
                           from res in opt.Match(
                               x  => result<char, double>(x),
                               () => failure<char, double>("Invalid floating point value")
                           )
                           select res;
                ...
                lexemeStr(
                    attempt(
                        ...
                        from op in def.ReservedOpNames.Contains(name)
                            ? unexpected<char, string>("reserved operator " + name)
                            : result<char, string>(name)
                        select op));
            ...
            var identifier = lexemeStr(
                attempt(
                    ...
                    from r in isReservedName(name)
                        ? unexpected<char, string>("reserved word " + name)
                        : result<char, string>(name)
                    select r));
        ...
        static Func<Parser<char, T>, Parser<char, T>> lexemeDef<T>(Parser<char, Unit> whiteSpace)
        {
            var ws = whiteSpace;
            return (Parser<char, T> p) =>
                from x in p
                from _ in ws
                select x;
        }

        static Parser<char, T> parseEsc<T>(char c, T code) =>
            from _ in ch(c)
            select code;

...
public abstract record ParseError<T, E>(long Offset) : Semigroup<ParseError<T, E>>
{
    /// <summary>
    ...
    /// and expected tokens.
    /// </summary>
    public record Trivial(long Offset, Option<ErrorItem<T>> Unexpected, Set<ErrorItem<T>> Expected)
    ...
    /// Fancy, custom errors.
    /// </summary>
    public record Fancy(long Offset, Set<ErrorFancy<E>> Errors)
        : ParseError<T, E>(Offset)
    {
        public override Func<State<S, T, E>, K<M, B>> WithHints<S, M, B>(
            Hints<T> hs, 

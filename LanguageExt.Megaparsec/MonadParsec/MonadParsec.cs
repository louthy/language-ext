...
/// <typeparam name="M">This monad</typeparam>
public interface MonadParsec<E, S, T, M> : 
    Monad<M>, 
    Alternative<M>,
    Identifiable<M, string>
    ...
    where S : TokenStream<S, T>
{
    ...
    /// <returns></returns>
    public static abstract K<M, A> Error<A>(ParseError<T, E> error);
    ...
    public static abstract K<M, A> Label<A>(string name, K<M, A> p);
    ...
    /// <typeparam name="A">Parser value type</typeparam>
    /// <returns>Parser</returns>
    public static virtual K<M, A> Hidden<A>(K<M, A> p) =>
        M.Label("", p);
    ...
    /// <typeparam name="A">Parser value type</typeparam>
    /// <returns>Parser</returns>
    public static abstract K<M, A> Try<A>(K<M, A> p);
    ...
    /// <typeparam name="A">Parser value type</typeparam>
    /// <returns>Parser</returns>
    public static abstract K<M, A> LookAhead<A>(K<M, A> p);
    ...
    public static abstract K<M, Unit> NotFollowedBy<A>(K<M, A> p);
    ...
    /// <typeparam name="A">Parser value type</typeparam>
    /// <returns>Parser</returns>
    public static abstract K<M, A> WithRecovery<A>(Func<ParseError<T, E>, K<M, A>> onError, K<M, A> p);

    /// <summary>
    /// This parser only succeeds at the end of input
    /// </summary>
    public static abstract K<M, Unit> EOF { get; }
    ...
    /// <typeparam name="A">Parser value type</typeparam>
    /// <returns>Parser</returns>
    public static abstract K<M, Either<ParseError<T,E>, A>> Observing<A>(K<M, A> p);
    ...
    public static abstract K<M, A> Token<A>(Func<T, Option<A>> test, in Set<ErrorItem<T>> expected);
    ...
    public static abstract K<M, S> Tokens(Func<S, S, bool> test, S chunk);
    ...
    public static abstract K<M, S> TakeWhile(Option<string> name, Func<T, bool> test);
    ...
    /// <param name="test">Predicate to use to test tokens</param>
    /// <returns>A chunk of matching tokens</returns>
    public static abstract K<M, S> TakeWhile1(Option<string> name, Func<T, bool> test);
    ...
    /// <returns>A chunk of matching tokens</returns>
    public static abstract K<M, S> Take(Option<string> name, int n);

    /// <summary>
    /// Return the full parser state as a `State` record
    /// </summary>
    /// <returns>Parser</returns>
    public static abstract K<M, State<S, T, E>> ParserState { get; }
    ...
    /// <returns>Parser</returns>
    public static abstract K<M, Unit> UpdateParserState(Func<State<S, T, E>, State<S, T, E>> f);
    ...
    /// <typeparam name="A">Parser value type</typeparam>
    /// <returns>Parser</returns>
    public static abstract K<M, A> Lift<A>(Func<State<S, T, E>, Reply<E, S, A>> f);
}

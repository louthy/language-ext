    ...
    MonadIO<StateIO<S>>
{
    ...
    static K<StateIO<S>, B> Maybe.MonadIO<StateIO<S>>.MapIO<A, B>(K<StateIO<S>, A> ma, Func<IO<A>, IO<B>> f) =>
    ...
    static K<StateIO<S>, IO<A>> Maybe.MonadIO<StateIO<S>>.ToIO<A>(K<StateIO<S>, A> ma) => 
        from s in Stateful.get<StateIO<S>, S>()
        let a = Atom(s)

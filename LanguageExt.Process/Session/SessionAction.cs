...
namespace LanguageExt.Session
{
    ...
    public class SessionAction
    {
        ...
        [JsonConstructor]
        public SessionAction(SessionActionTag tag, long time, SessionId sessionId, string systemName, string nodeName, int timeout, string key, string value)
        {
            Tag = tag;
            Time = time;
            SessionId = sessionId;
        ...
        public static SessionAction Touch(SessionId sessionId, SystemName systemName, ProcessName nodeName) =>
            new SessionAction(SessionActionTag.Touch, 0L, sessionId, systemName.Value, nodeName.Value, 0, null, null);

        public static SessionAction Stop(SessionId sessionId, SystemName systemName, ProcessName nodeName) =>
            new SessionAction(SessionActionTag.Stop, 0L, sessionId, systemName.Value, nodeName.Value, 0, null, null);

        public static SessionAction Start(SessionId sessionId, int timeoutSeconds, SystemName systemName, ProcessName nodeName) =>
            new SessionAction(SessionActionTag.Start, 0L, sessionId, systemName.Value, nodeName.Value, timeoutSeconds, null, null);
        ...
        public static SessionAction ClearData(long time, SessionId sessionId, string key, SystemName systemName, ProcessName nodeName) =>
            new SessionAction(SessionActionTag.ClearData, time, sessionId, systemName.Value, nodeName.Value, 0, key, null);
        ...
        public static SessionAction SetData(long time, SessionId sessionId, string key, string data, SystemName systemName, ProcessName nodeName) =>
            new SessionAction(SessionActionTag.SetData, time, sessionId, systemName.Value, nodeName.Value, 0, key, data);
    }
}

...
public class Pipe<RT, IN, OUT> : MonadT<Pipe<RT, IN, OUT>, Eff<RT>>
{
    ...
    static K<Pipe<RT, IN, OUT>, B> MonadIO<Pipe<RT, IN, OUT>>.MapIO<A, B>(K<Pipe<RT, IN, OUT>, A> ma, Func<IO<A>, IO<B>> f) => 
        ma.As().MapIO(f);

    static K<Pipe<RT, IN, OUT>, IO<A>> MonadIO<Pipe<RT, IN, OUT>>.ToIO<A>(K<Pipe<RT, IN, OUT>, A> ma) => 
        ma.MapIO(IO.pure);
}
